...
function printBtn() {
    for (var i = 0; i < bezierCurves.length; i++) {
        ...
        var t = document.createTextNode(bezierCurves[i]);
        btn.appendChild(t);
...
window.onload = function() {
  ...
  console.log(boardPassGenerator.numberSteps)
  ...
  function buttonEvent(isIncrement, target){
    ...
    if (isIncrement == true) {
        addButton(boardPassGenerator)
        boardPassGenerator.numberSteps += 1
    }
    else {
        subtractButton(boardPassGenerator)
        boardPassGenerator.numberSteps -= 1
    }
    
  }

  function addButton(boardPassGenerator) { 
    var div = document.getElementById('div'); 
    // console.log(div)
    var ul = document.getElementById("list");
    var li = document.createElement("li")
    li.setAttribute("class", 'vert-li');
    // console.log(li);
    li.appendChild(document.createTextNode(boardPassGenerator.numberSteps));
    ul.appendChild(li);
    console.log(boardPassGenerator.numberSteps)

  }

  function subtractButton(boardPassGenerator) { 
    var ul = document.getElementById("list");
